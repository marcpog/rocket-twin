...
class Ground(System):
    ...
    def setup(self, stations=None):
        ...
        for station in stations:
            ...
            self.add_child(Station(station, n_stages=1), pulling = {"a" : f"a_{station}", "pos" : f"pos_{station}"})
            self.add_transient(f"v_{station}", der=f"a_{station}")
            self.add_transient(f"r_{station}", der=f"v_{station}")
